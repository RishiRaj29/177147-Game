...
class Game(tk.Frame):
    ...
    def start_game(self):
        ...
        self.cells[row][col]["frame"].configure(bg=c.CELL_COLORS[3])
        self.cells[row][col]["number"].configure(
            bg=c.CELL_COLORS[3],
            fg=c.CELL_NUMBER_COLORS[3],
            font=c.CELL_NUMBER_FONTS[3],
            text="3")
    ...
    def add_new_tile(self):
        ...
        while(self.matrix[row][col] != 0):
            row = random.randint(0, 3)
            col = random.randint(0, 3)
        self.matrix[row][col] = random.choice([3, 9])
    ...
    def game_over(self):
        if any(177147 in row for row in self.matrix):
